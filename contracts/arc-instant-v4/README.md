...
Basket pairs (`BasketShareVault` / `BasketVaultFactory`) are fixed-quantity ERC-20 shares, same
accounting as Blend: one share maps to a set amount of each asset, and prices do not rebalance
the tokens. A mint pulls that amount rounded up, plus an owner fee (0–1%) and a 0.35% protocol
fee, on each asset. A redeem pays the amount rounded down, minus the owner fee and a 0.20%
protocol fee. Holder backing, owner treasury, and protocol fees are separate buckets. The share
is the quote on `RwaInstantV4Factory`. Deploy with `script/DeployBasketVaultFactory.s.sol`, then
