...
## Holder payout bundles — `BundleVault.sol`

"Earn RWAs automatically, just for holding," from [based.bid](https://x.com/basedbidx/status/2081029361965080803): a creator-configured bundle of assets (stocks, ETFs, other RWAs) that a
...
- **One `BundleVault` per pool, never a shared address.** `RwaFeeHook.owed[recipient][currency]`
  ...
  same slot with no way to tell whose money is whose. `RwaInstantV4Factory.createTokenWithBundleVault`
...
- **`convert(fromCurrency, minOuts)`** — swaps the pulled balance into the bundle via real v4
  pools (creator supplies the pool key per bundle asset at `setBundle` time — mismatches revert
...
- **`setBundle(...)` is creator-only and callable any time** — "fully automatic, no relaunches":
  changing what holders earn never touches the token or its pool.

Proven by `test/BundleVault.t.sol` (10 tests, part of the same `forge test` run below): real
...
### The off-chain keeper — `scripts/bundle-vault-keeper.ts`

Reads a vault's live bundle/pending balances, pulls whatever `RwaFeeHook` has accrued to it,
...
bundle-enabled launch, two seeded RWA-mock pools) to a local Anvil node via a real
...
holders receiving real bundle-asset balances matching their pro-rata share.
...
21/21 passing (11 for the launch path, 10 for BundleVault), including a 256-run fuzz test that
