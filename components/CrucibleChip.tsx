/**
 * Thin Crucible status chip. Live last-burn copy only when real events exist.
 */
'use client'
...
export function CrucibleChip({
  ...
  const label = compact ? 'Crucible' : 'Burn tape · no burns yet'
  return (
    <Link
      href="/crucible"
      title="Crucible — buy $EVE from quote fees and burn it"
    ...
    >
      ...
      <span className="whitespace-nowrap tracking-tightish tabular-nums">{label}</span>
    </Link>
  )
}
