# Eve + Circle Agent Stack + eve.fun

eve.fun MCP is the venue. Circle Agent Stack is the wallet. Eve is the agent.
...
Run `circle blockchain list` if `ARC` is not accepted. Circle currently publishes Arc testnet as `ARC-TESTNET`. On live eve.fun (chain 5042) use the Arc identifier your CLI lists, or a Circle local wallet plus `--rpc-url`.

## Flow

1. Eve discovers eve.fun tools through `connections/arcfun.ts` (`defineMcpClientConnection`).
...
eve.fun never sees the private key. Circle holds the Agent Wallet under your policy. Eve is the runtime.
