...
test('mainnet catalog bakes USYC + cirBTC + XAUM CAs; Instant-create ready via shared factory', async () => {
  ...
  // $5500 at $4000/oz → 1.375 XAUM = 1.375e18 raw (18dp)
  assert.equal(defaultRwaVirtualQuoteRaw(xaum, { spotUsd: 4_000 }), 1_375_000_000_000_000_000n)
  assert.equal(defaultRwaVirtualQuoteRaw(xaum), 1_375_000_000_000_000_000n)
  ...
  assert.equal(defaultRwaVirtualQuoteRaw(cirbtc, { btcUsd: 100_000 }), 5_500_000n)
  assert.equal(defaultRwaVirtualQuoteRaw(cirbtc, { btcUsd: 50_000 }), 11_000_000n)
  // Missing spot: 0.055 cirBTC (~$5.5k at $100k), not 55 cirBTC.
  assert.equal(defaultRwaVirtualQuoteRaw(cirbtc), 5_500_000n)
  assert.notEqual(defaultRwaVirtualQuoteRaw(cirbtc), 5_500_000_000n)
...
test('every catalog quote has an explicit USD policy (no silent USDC 6dp)', async () => {
  ...
  assert.equal(defaultRwaVirtualQuoteRaw(usyc), 5500n * 10n ** 6n)
  assert.equal(crcl.usd, 'none')
  assert.equal(quoteUsesUsdInput(crcl), false)
  // 8dp without usd=spot must not look like USDC 5500e6.
  const fake8 = { ...cirbtc, id: 'wbtc', usd: 'none', usdSpot: undefined, payUsdcSwap: false, decimals: 8 }
  assert.equal(defaultRwaVirtualQuoteRaw(fake8), 5500n * 10n ** 8n)
  assert.notEqual(defaultRwaVirtualQuoteRaw(fake8), 5_500_000_000n)
})
