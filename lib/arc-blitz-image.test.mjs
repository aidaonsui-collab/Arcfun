...
test('plain retweets are ignored, quote-reposts are not', () => {
  assert.equal(isRetweetRef([{ type: 'retweeted', id: '1' }]), true)
  assert.equal(isRetweetRef([{ type: 'quoted', id: '1' }]), false)
  assert.equal(isRetweetRef([{ type: 'replied_to', id: '1' }]), false)
  assert.equal(isRetweetRef([]), false)
})
