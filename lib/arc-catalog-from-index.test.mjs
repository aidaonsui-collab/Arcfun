...
test('newer indexer lastTradeAt wins on volume; Instant slot0 keeps price/mcap', () => {
  const rpc = [
    {
      id: '0xaaa',
      coinType: '0xaaa',
  ...
  assert.equal(t?.currentPrice, 6.992e-5)
  assert.equal(t?.marketCap, 69_920)
...
test('catalogSpotFromLiveOrTape prefers slot0 over a later tape print', () => {
  const live = catalogSpotFromLiveOrTape(8.35e-5, 2.047e-4)
  assert.equal(live.currentPrice, 8.35e-5)
  assert.equal(live.marketCap, 8.35e-5 * 1_000_000_000)
  const tapeOnly = catalogSpotFromLiveOrTape(0, 2.047e-4)
  assert.equal(tapeOnly.currentPrice, 2.047e-4)
})
...
test('merge does not copy a 6dp-as-18 tape print over Instant slot0', () => {
  ...
  assert.equal(fmtUsd(merged[0].marketCap), '$7.1K')
})
